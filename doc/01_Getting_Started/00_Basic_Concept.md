...
With PimPrint you create elements in an InDesign file, acting as the template for document generation defining fonts and styles. When rendering a document, elements from the
template file are copied into the generated InDesign document. While placing the elements, they can be freely positioned, resized and filled with content. This is done by
...
Open the _Element name_ tab, select an InDesign page element, enter a name and click the save button.

![Basic Concept - Element name](../img/basic-concept-element_name.png)

The example assigns the identifier `templateElement` to the red square to use it in the generation of rendering instructions for the plugin.
...
document. All content, positioning and placement are implemented serverside.
...
The following example copies the above create element `templateElement` from the template file into the generated document at the fixed position _top: 10 mm / left: 10 mm_.
